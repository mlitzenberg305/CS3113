...
void Captured::Update(float deltaTime) {
}

void Captured::Render(ShaderProgram *program) {
    
    program->SetProjectionMatrix(uiProjectionMatrix);
    program->SetViewMatrix(uiViewMatrix);
    
    Util::DrawText(program, state.fontTextureID, "Animal Control", 0.5, -0.2f, glm::vec3(-3, 1, 0), 0);
    Util::DrawText(program, state.fontTextureID, "captured you...", 0.5, -0.2f, glm::vec3(-3, 0, 0), 0);
