...
void Initialize() {
    ...
    for (int i = 0; i < PLATFORM_COUNT; i++) {
        ...
        state.platforms[i].position = glm::vec3((float)(i - 1), -3.25f, 0.0f);
        state.platforms[i].animStop = new int[1] {22};
        
        state.platforms[i].animIndices = state.platforms[i].animStop;
        state.platforms[i].animFrames = 1;
        state.platforms[i].animIndex = 0;
        state.platforms[i].animTime = 0;
        state.platforms[i].animCols = 20;
        state.platforms[i].animRows = 9;
